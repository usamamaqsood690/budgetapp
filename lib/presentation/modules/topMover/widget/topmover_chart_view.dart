// lib/presentation/modules/topMover/widget/topmover_chart_view.dart
...
import 'chart_shimmer.dart';



class TopMoverChartView extends StatelessWidget {
  const TopMoverChartView({
    super.key,
    required this.controller,
    this.showLineInfoDialog = false,
    this.lineInfoDialogBuilder,
  });

  final TopMoverChartInterface controller;
  final bool showLineInfoDialog;
  final Widget Function(BuildContext context)? lineInfoDialogBuilder;
      ...
      return Stack(
        children: [
          controller.isChartFit.value
            ...
            showLineInfoDialog: showLineInfoDialog,
            lineInfoDialogBuilder: lineInfoDialogBuilder,
          )
            ...
            child: Column(
              children: [
                _ChartIconButton(
                  ...
                  onTap: () => Get.to(
                        () => TopMoverFullScreenChart(controller: controller),
                  ),
                ),
...
class _FitChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final double scale =
          _scaleFactors[controller.selectedRange.value] ?? 0.8;
      double calcWidth = (available / dataLength).clamp(0.5, 10.0) * scale;

      final clamp =
          _widthRanges[controller.selectedRange.value] ?? [0.3, 8.0];
      ...
      return Obx(() {
        ...
        if (isLine && showLineInfoDialog && lineInfoDialogBuilder != null) {
          return Stack(
            children: [
              KChartWidget(
...
class _ZoomChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final style = ChartStyle()..dateTimeFormat = controller.chartDateFormat;
      return KChartWidget(
        controller.chartData, style, _chartColors(),
        ...
        volHidden: true,
        isTapShowInfoDialog:
        controller.chartMode.value == ChartMode.candle,
        hideGrid: false,
        timeFormat: TimeFormat.yearMONTHDAY,
      );
    });
...
class TopMoverChartTabs extends StatelessWidget {
  const TopMoverChartTabs({super.key, required this.controller});
  final TopMoverChartInterface controller;

  static const List<String> _ranges = [
    '1 D', '7 D', '1 M', '6 M', '1 Y', 'YTD'
  ];
...
class TopMoverFullScreenChart extends StatefulWidget {
  const TopMoverFullScreenChart({super.key, required this.controller});
  final TopMoverChartInterface controller;

  @override
  State<TopMoverFullScreenChart> createState() =>
      _TopMoverFullScreenChartState();
}

class _TopMoverFullScreenChartState extends State<TopMoverFullScreenChart> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Obx(() {
          final style = ChartStyle()
            ..dateTimeFormat = widget.controller.chartDateFormat;
          ...
          return Stack(
            children: [
              KChartWidget(
                widget.controller.chartData, style, colors,
                isLine:
                widget.controller.chartMode.value == ChartMode.line,
                isTrendLine: false,
                volHidden: true,
                isTapShowInfoDialog: true,
                hideGrid: false,
